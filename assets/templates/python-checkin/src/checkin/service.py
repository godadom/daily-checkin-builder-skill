...
class CheckinService:
    ...
    def _query(self, account: AccountConfig) -> tuple[CheckinResult | None, str | None, bool]:
        try:
        ...
        if response.status == 403:
            if code in self.ACCESS_DENIED_CODES:
                return CheckinResult(account.name, CheckinStatus.UNSUPPORTED_SECURITY_CHALLENGE, "server denied access; do not retry or bypass"), None, False
    ...
    def _confirm_after_ambiguous(self, account: AccountConfig) -> CheckinResult:
        ...
        if result and result.status in {CheckinStatus.AUTH_EXPIRED, CheckinStatus.UNSUPPORTED_SECURITY_CHALLENGE, CheckinStatus.SITE_CHANGED}:
            return result
    ...
    def check_in(self, account: AccountConfig) -> CheckinResult:
        ...
        try:
            response = self.client.request("POST", self.settings.base_url + self.settings.checkin_path, headers, body)
        except ResponseTooLarge:
        ...
        if response.status == 403:
            if code in self.ACCESS_DENIED_CODES:
                return CheckinResult(account.name, CheckinStatus.UNSUPPORTED_SECURITY_CHALLENGE, "server denied access; do not retry or bypass")
