...
Create panel environment variables from `.env.example`. Required values are `CHECKIN_BASE_URL` plus either the single-account authentication fields or one `CHECKIN_ACCOUNTS` JSON array. The origin and paths must come from an authorized, sanitized site analysis. Store secrets only in QingLong's protected environment-variable store, disable any value that is no longer used, and never paste secrets into logs or commands.
