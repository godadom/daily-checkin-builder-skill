...
若站点提供账号密码、OTP、二维码或其他交互登录，不要在 Actions 定时工作流中执行登录或显示浏览器。让操作者在本机手动运行 headed 的站点专用登录入口，在页面中输入账号密码/验证码并完成人机验证；只有本机已安装并登录 GitHub CLI 时，才允许程序通过标准输入调用 `gh secret set` 更新目标 Secret。禁止使用命令参数、工作流输出、artifact 或日志传递任何秘密。
